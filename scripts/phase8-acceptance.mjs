...
import { buildDocx, buildPptx, buildRtf, buildXlsx, layoutPage, tableRows } from "../src/features/convert/ooxml.ts";
...
async function sourcePdf() {
  ...
  first.drawText("Revenue grew in every region during the quarter.", { x: 72, y: 680, size: 11, font });
  ...
  table.forEach((row, r) =>
    row.forEach((cell, c) => first.drawText(cell, { x: 72 + c * 170, y: 600 - r * 18, size: 11, font })),
  );
...
async function layouts(bytes) {
  ...
  for (let number = 1; number <= doc.numPages; number++) {
    ...
    pages.push(layoutPage(number, content.items.filter((item) => typeof item.str === "string"), width, height));
  }
  return pages;
}
...
await writeFile(files.xlsx, buildXlsx(pages.map((page) => ({ name: `Page ${page.page}`, rows: tableRows(page) }))));
await writeFile(
  files.pptxText,
  buildPptx(
    pages.map((page) => ({
...
record("textutil reads the DOCX heading and body", docxText.includes("Quarterly Report 2026") && docxText.includes("Revenue grew in every region"));
record("textutil reads accented DOCX text", docxText.includes("Café résumé naïve"));
record("DOCX keeps column reading order", docxText.indexOf("Left 11") >= 0 && docxText.indexOf("Left 11") < docxText.indexOf("Right 0"));
const rtfText = run("textutil", ["-convert", "txt", "-stdout", files.rtf]).stdout;
record("textutil reads the RTF with Unicode escapes", rtfText.includes("Quarterly Report 2026") && rtfText.includes("Café résumé naïve"));

const sheet = run("python3", ["-c", "import zipfile,sys; print(zipfile.ZipFile(sys.argv[1]).read('xl/worksheets/sheet1.xml').decode())", files.xlsx]).stdout;
record("XLSX stores 1,250.50 as a number", /<c r="B\d+"><v>1250.5<\/v><\/c>/.test(sheet));
record("XLSX stores the ISO date as a dated serial", /<c r="C\d+" s="1"><v>46279<\/v><\/c>/.test(sheet));
record("XLSX keeps formula-like text as an inline string", sheet.includes("<t xml:space=\"preserve\">=SUM(A1:A2)</t>") && !sheet.includes("<f>"));

const pptxNames = packageCheck(files.pptxText).names ?? [];
record("PPTX has one slide per page", pptxNames.filter((name) => /^ppt\/slides\/slide\d+\.xml$/.test(name)).length === pages.length);
...
record("Quick Look renders the PPTX, DOCX and XLSX", thumbnails.length === 3, thumbnails.join(", "));
