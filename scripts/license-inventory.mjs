...
function permissive(expression) {
  ...
  const tokens = expression.replace(/\s*\/\s*/g, " OR ").match(/\(|\)|[^\s()]+(?:\s+WITH\s+[^\s()]+)?/g) ?? [];
  let position = 0;
  const primary = () => {
    if (tokens[position] === "(") {
      position++;
...
const crates = metadata.packages
  ...
  .map((pkg) => ({ name: pkg.name, version: pkg.version, license: pkg.license ?? null, repository: pkg.repository ?? null }))
  .sort((a, b) => a.name.localeCompare(b.name) || a.version.localeCompare(b.version));

const lock = JSON.parse(readFileSync("package-lock.json", "utf8"));
const npm = Object.entries(lock.packages)
  .filter(([key, value]) => key && !value.dev)
  .map(([key, value]) => ({ name: key.replace(/^.*node_modules\//, ""), version: value.version, license: value.license ?? null, optional: !!value.optional }))
  .sort((a, b) => a.name.localeCompare(b.name));

const review = [
  ...crates.filter((item) => !permissive(item.license)).map((item) => ({ ecosystem: "cargo", ...item })),
  ...npm.filter((item) => !permissive(item.license)).map((item) => ({ ecosystem: "npm", ...item })),
];
const count = (items) =>
  Object.entries(items.reduce((totals, item) => ({ ...totals, [item.license ?? "UNKNOWN"]: (totals[item.license ?? "UNKNOWN"] ?? 0) + 1 }), {}))
    .sort((a, b) => b[1] - a[1]);

const report = {
  target: TARGET,
  project: { cargoLicense: metadata.packages.find((pkg) => workspace.has(pkg.id))?.license ?? null, npmLicense: JSON.parse(readFileSync("package.json", "utf8")).license ?? null },
...
console.log(`${crates.length} crates and ${npm.length} npm packages; ${review.length} need license review; wrote ${output}`);
for (const item of review) console.log(`  review ${item.ecosystem} ${item.name} ${item.version}: ${item.license ?? "no license field"}`);
console.log(`project license: cargo ${report.project.cargoLicense ?? "not declared"}, npm ${report.project.npmLicense ?? "not declared"}`);
