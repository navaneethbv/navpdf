...
function leaf(name, serial, keyArgs, validity) {
  tool("openssl", [
    "req", "-new", "-newkey", ...keyArgs, "-noenc",
    "-keyout", key(`${name}.key`), "-out", key(`${name}.csr`),
    "-subj", `/CN=Synthetic ${name} signer/O=NavPDF Acceptance`,
  ]);
  tool("openssl", [
    "x509", "-req", "-in", key(`${name}.csr`), "-CA", key("root.pem"), "-CAkey", key("root.key"),
    "-set_serial", String(serial), ...validity, "-extfile", key("leaf.ext"), "-out", key(`${name}.pem`),
  ]);
}

function pkcs12(name, source, extra = []) {
  tool("openssl", [
    "pkcs12", "-export", "-inkey", key(`${source}.key`), "-in", key(`${source}.pem`),
    "-certfile", key("root.pem"), "-name", name, ...extra,
    "-passout", "env:NAVPDF_ACCEPTANCE_P12", "-out", key(`${name}.p12`),
  ]);
}
...
function signPdf(label, source, identity, options = {}, password = secret) {
  ...
  writeFileSync(
    request,
    JSON.stringify({ identity: key(`${identity}.p12`), reason: "", location: "", certification: "none", ...options }),
  );
...
function opensslVerify(file) {
  ...
  for (const match of text.matchAll(/\/ByteRange\s*\[\s*(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s*\]/g)) {
    ...
    const result = run("openssl", [
      "cms", "-verify", "-binary", "-inform", "DER", "-in", cms, "-content", content,
      "-CAfile", key("root.pem"), "-purpose", "any", "-out", "/dev/null",
    ]);
...
function examine(label, file, source, count, certification = null) {
  ...
  check(`${label}: earlier revision bytes are preserved`, signed.subarray(0, original.length).equals(original));
  check(
  ...
  check(`${label}: pdfsig finds ${count} signature(s)`, poppler.length === count, String(poppler.length));
  check(`${label}: pdfsig reports every signature valid`, poppler.length > 0 && poppler.every((entry) => entry.valid));
  check(`${label}: pdfsig reports ETSI.CAdES.detached covering the whole file`, newest?.etsi && newest.total);
  check(`${label}: pdfsig trusts the synthetic root certificate`, newest?.trusted);
  const cms = opensslVerify(file);
  check(`${label}: OpenSSL verifies every CMS signature`, cms.length === count && cms.every(Boolean));
...
try {
  ...
  tool("openssl", [
    "req", "-x509", "-newkey", "rsa:3072", "-noenc", "-keyout", key("root.key"), "-out", key("root.pem"),
    "-days", "30", "-subj", "/CN=NavPDF Synthetic Root/O=NavPDF Acceptance",
  ...
  pkcs12("rsa", "rsa");
  pkcs12("ec", "ec");
  pkcs12("legacy", "rsa", ["-keypbe", "PBE-SHA1-3DES", "-certpbe", "PBE-SHA1-3DES", "-macalg", "sha1"]);
  let expired = true;
  try {
    leaf("expired", 4099, ["rsa:2048"], ["-not_before", "20250101000000Z", "-not_after", "20250201000000Z"]);
  ...
  tool("certutil", ["-A", "-d", `sql:${NSS}`, "-n", "NavPDF Synthetic Root", "-t", "CT,C,C", "-i", key("root.pem")]);
  tool("cargo", ["build", "--manifest-path", "src-tauri/Cargo.toml", "--example", "engine_cli"]);
  for (const source of Object.values(SOURCES)) copyFileSync(source, path.join(OUT, path.basename(source)));
  ...
  const certified = signPdf("forms-certified", SOURCES.forms, "rsa", { certification: "formFilling" });
  ...
  check("P-256 identity countersigns the certified copy", countersigned.status === 0, countersigned.stderr);
  examine("forms-countersigned", countersigned.output, certified.output, 2, 2);
  const layered = pdfsig(countersigned.output);
  check("the first signature no longer covers the countersigned file", layered.length === 2 && !layered[0].total);

  const wrong = signPdf("wrong-password", SOURCES.reader, "rsa", {}, "not-the-synthetic-secret");
  check("a wrong PKCS #12 password is refused without output", wrong.status !== 0 && !existsSync(wrong.output));
  if (expired) {
    const late = signPdf("expired", SOURCES.reader, "expired");
    check("an expired certificate is refused", late.status !== 0 && /expired/.test(late.stderr), late.stderr);
  }
  const recertify = signPdf("recertify", rsa.output, "rsa", { certification: "noChanges" });
  check("certifying an already signed file is refused", recertify.status !== 0 && /first signature/.test(recertify.stderr));
  ...
  check("pdfsig rejects a byte changed inside the signed range", pdfsig(tamperedFile).every((entry) => !entry.valid));
  check("OpenSSL rejects the tampered ranges", opensslVerify(tamperedFile).every((passed) => !passed));
  check("NavPDF reports the tampered copy as changed", navpdfReport(tamperedFile)?.[0]?.status === "modified");

  const appendedFile = path.join(OUT, "appended.pdf");
  writeFileSync(appendedFile, Buffer.concat([readFileSync(rsa.output), Buffer.from("\n% synthetic later change\n")]));
  const appended = pdfsig(appendedFile);
  check("pdfsig keeps an appended copy valid but not totally signed", appended[0]?.valid && !appended[0].total);
  ...
  for (const name of ["rsa", "ec", "legacy", "expired"]) rmSync(key(`${name}.p12`), { force: true });
