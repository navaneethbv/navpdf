...
Preview access was available in the follow-up desktop session; see the verification ledger for observed rendering, persistence, search and outline results and remaining reader UI limits.
...
The follow-up Preview inspection supplements PDF.js/Poppler checks; Apple PDFKit independently confirms the saved single-root outlines that Preview displays under the filename.
