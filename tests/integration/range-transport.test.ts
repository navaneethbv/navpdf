...
vi.mock("../../src/services/native", () => ({
  readRange: async (_id: string, begin: number, end: number) => {
    ...
    if (end - begin > 4 * 1024 * 1024)
      throw new Error("Range exceeded native limit");
...
describe("local byte range transport", () => {
  it("delivers coalesced ranges as one response while bounding native reads", async () => {
    ...
    transport.transportReady(
      (event: { type: string; begin: number; chunk: Uint8Array }) => {
    ...
    expect(
      reads.requests.every(([begin, end]) => end - begin <= 1024 * 1024),
    ).toBe(true);
  });
  it("loads and extracts remote pages through the same range adapter as the desktop app", async () => {
    reads.bytes = new Uint8Array(
      await readFile("tests/pdf-fixtures/reader-1000.pdf"),
    );
    reads.requests = [];
    const errors: Error[] = [];
    ...
    try {
      ...
      const text = (
        await page.getTextContent({ disableNormalization: true })
      ).items
