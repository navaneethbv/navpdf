...
export function nearestStandardStyle(font: FontDescription): StandardTextStyle {
  ...
  let face: TextFontFace = "sans";
  if (MONO.test(name) || font.fallbackName === "monospace") face = "mono";
  else if (/sans/i.test(name)) face = "sans";
  else if (SERIF.test(name) || font.fallbackName === "serif") face = "serif";
  return {
    face,
...
function glyphText(glyphs: unknown): string {
  ...
  return glyphs
    .map((glyph) =>
      glyph && typeof glyph === "object" && "unicode" in glyph ? String(glyph.unicode) : "",
    )
    .join("");
}
  ...
  for (const [index, fn] of fnArray.entries()) {
    ...
    switch (fn) {
      ...
      case OPS.setTextMatrix:
        textMatrix = asMatrix(args) ?? asMatrix(args[0]) ?? textMatrix;
        break;
      case OPS.showText:
      case OPS.showSpacedText:
      case OPS.nextLineShowText:
      case OPS.nextLineSetSpacingShowText: {
        if (state.renderMode === INVISIBLE_TEXT || !state.font) break;
        const text = glyphText(args[args.length - 1]);
        if (!text.trim()) break;
        const combined = multiply(textMatrix, state.ctm);
        const size = Math.round(state.fontSize * Math.hypot(combined[2], combined[3]) * 2) / 2;
        if (size > 0) runs.push({ font: state.font, size, color: state.fill, text });
        break;
      }
    }
  }
